...
interface DestinationCardProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  flag: string;
  stats: string;
  href?: string; // Optional for this integration
  onClick?: () => void; // Added for role selection
  themeColor: string; // e.g., "150 50% 25%" for a deep green
}

const DestinationCard = React.forwardRef<HTMLDivElement, DestinationCardProps>(
  ({ className, imageUrl, location, flag, stats, href, onClick, themeColor, ...props }, ref) => {
    ...
    return (
      ...
      >
        <div
          ...
          aria-label={`Select role: ${location}`}
          style={{
        ...
        >
          ...
          <div className="relative flex flex-col justify-end h-full p-6 text-white">
            ...
            <div className="mt-8 flex items-center justify-between bg-[hsl(var(--theme-color)/0.2)] backdrop-blur-md border border-[hsl(var(--theme-color)/0.3)] 
              ...
              <span className="text-sm font-semibold tracking-wide">Enter Dashboard</span>
