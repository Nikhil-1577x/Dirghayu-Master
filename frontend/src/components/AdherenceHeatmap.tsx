...
import { useApiStore, useMedicationStore, type AdherenceDay } from '../store';
import { getAdherenceHistory } from '../api/endpoints';
...
export default function AdherenceHeatmap() {
  ...
  const [history, setHistory] = useState<AdherenceDay[]>(adherenceData);
  const svgRef = useRef<SVGSVGElement>(null);
  const tooltipRef = useRef<HTMLDivElement>(null);
  const todayIso = new Date().toISOString().slice(0, 10);

  const historyRangeLabel = useMemo(() => {
    if (!history.length) return '365-day history';
    const start = new Date(history[0].date);
    const end = new Date(history[history.length - 1].date);
    const startYear = start.getFullYear();
    const endYear = end.getFullYear();
    return startYear === endYear ? `365-day history (${startYear})` : `365-day history (${startYear}-${endYear})`;
  }, [history]);

  useEffect(() => {
    let active = true;
    if (!patientId) {
      setHistory(adherenceData);
      return () => {
        active = false;
      };
    }
    (async () => {
      try {
        const resp = await getAdherenceHistory(patientId, 365);
        if (!active) return;
        setHistory((resp.history || []) as AdherenceDay[]);
      } catch {
        if (!active) return;
        // Safe fallback to existing in-memory data to avoid breaking UI.
        setHistory(adherenceData);
      }
    })();
    return () => {
  ...
  useEffect(() => {
    ...
    history.forEach((item) => {
      ...
      const diffDays = Math.round((date.getTime() - new Date(history[0].date).getTime()) / 86400000);
      ...
      if (date.getDate() <= 7) {
        ...
        if (!monthLabeled.has(monthKey)) {
          ...
          const monthText =
            lastLabeledYear === null || lastLabeledYear !== date.getFullYear()
              ? `${date.toLocaleString('default', { month: 'short' })} '${String(date.getFullYear()).slice(-2)}`
              : date.toLocaleString('default', { month: 'short' });
          lastLabeledYear = date.getFullYear();
          svg
            .append('text')
            .attr('x', x + cellSize / 2)
            .attr('y', marginTop - 8)
      ...
      svg
        ...
        .attr('ry', 4)
        .attr('fill', color)
        .attr('stroke', item.date === todayIso ? '#0f766e' : '#cbd5e1')
        .attr('stroke-width', item.date === todayIso ? 1.5 : 0.6)
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{historyRangeLabel}</div>
          </div>
        </div>
