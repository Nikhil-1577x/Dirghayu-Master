...
import { downloadPatientSummaryReport } from '../api/endpoints';
...
export default function AppointmentCard() {
  const { appointments, patientId } = useApiStore();
  ...
  const onDownload = async () => {
    if (!patientId) {
      window.alert('Please select a patient first.');
      return;
    }
    try {
      setDownloading(true);
      await downloadPatientSummaryReport(patientId);
    } catch (e) {
    ...
    <div className="card">
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
          ...
          >
            <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--accent-primary)', lineHeight: 1.1 }}>{Math.abs(days)}</div>
      ...
      >
        ...
        {downloading ? 'Generating Report...' : 'Generate PDF Summary Report'}
      </button>
    </div>
  );
}
