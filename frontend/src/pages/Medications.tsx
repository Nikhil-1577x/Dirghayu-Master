...
import { Pill, Plus, Calendar, CheckSquare, Activity } from 'lucide-react';
import { useIotStore, useMedicationStore } from '../store';
...
export default function Medications() {
  ...
  const hardwareHistory = useIotStore((s) => s.history);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28, paddingBottom: 48 }}>
      ...
      {/* Hardware Dose History */}
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)' }}>Hardware Dose History</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>
            Persistent logs of all Missed and Taken events generated by the smart dispenser
          </div>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)' }}>
              <th style={{ padding: '14px 24px', fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Date & Time</th>
              <th style={{ padding: '14px 24px', fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Hardware Slot</th>
              <th style={{ padding: '14px 24px', fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Status</th>
            </tr>
          </thead>
          <tbody>
            {hardwareHistory.slice(0, 8).map((row, i) => {
              const isMissed = row.status === 'missed';
              const isDispensed = row.status === 'dispensed';
              const color = isMissed ? '#ef4444' : isDispensed ? '#10b981' : '#f59e0b';
              const label = isMissed ? 'MISSED' : isDispensed ? 'TAKEN' : String(row.status).toUpperCase();
              return (
                <tr key={`${row.id}-${row.timestamp}`} style={{ borderBottom: i === Math.min(hardwareHistory.length, 8) - 1 ? 'none' : '1px solid var(--border)' }}>
                  <td style={{ padding: '16px 24px', fontSize: 14, color: 'var(--text-primary)' }}>{new Date(row.timestamp).toLocaleString()}</td>
                  ...
                  <td style={{ padding: '16px 24px' }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color, background: `${color}15`, border: `1px solid ${color}30`, padding: '4px 10px', borderRadius: 9999 }}>
                      {label}
                    </span>
                  </td>
                </tr>
            ...
            {hardwareHistory.length === 0 && (
              <tr>
                <td colSpan={3} style={{ padding: '16px 24px', color: 'var(--text-muted)', fontSize: 13 }}>
                  No hardware dose history available yet.
                </td>
              </tr>
            )}
          </tbody>
