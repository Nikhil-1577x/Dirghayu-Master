...
export function useApi() {
  ...
  const hydrateDashboard = useCallback(async (pid: number) => {
    ...
    try {
      ...
      // Keep timestamp-level fidelity so multiple same-day uploads are visible.
      ...
      for (const b of biomarkers as Array<{ biomarker_type: string; value: number; timestamp: string }>) {
        ...
        byTimestamp[ts][key] = b.value;
      }
      const bioList = Object.entries(byTimestamp)
        .map(([timestamp, rest]) => ({ date: timestamp.split('T')[0], timestamp, ...rest }))
